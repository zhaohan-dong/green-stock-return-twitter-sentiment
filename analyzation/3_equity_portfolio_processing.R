...
clean_stock_daily_return <- returns(clean_stock)
oil_gas_stock_equal_weight <- create_portfolio(oil_gas_stock) %>%
  read.zoo(FUN = as.Date) %>%
  returns()
utility_stock_equal_weight <- create_portfolio(utility_stock) %>%
  read.zoo(FUN = as.Date) %>%
  returns()
